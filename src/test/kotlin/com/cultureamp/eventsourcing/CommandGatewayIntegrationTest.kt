...
class CommandGatewayIntegrationTest : DescribeSpec({
    ...
    describe("CommandGateway") {
        it("accepts a creation event") {
            val result = gateway.dispatch(CreateClassicPizza(PizzaStyle.MARGHERITA, UUID.randomUUID()), EmptyMetadata())
        ...
        it("fails on creation with duplicate UUIDs") {
            ...
            val result = gateway.dispatch(CreateClassicPizza(PizzaStyle.MARGHERITA, aggregateId), EmptyMetadata())
            result shouldBe Right(Created)
            val result2 = gateway.dispatch(CreateClassicPizza(PizzaStyle.HAWAIIAN, aggregateId), EmptyMetadata())
